            ...
            if rst = '1' then
                ...
                if pattern_go = '1' and generating_pattern = '0' then
                    generating_pattern <= '1';
                        ...
                        if current_row = n_rows - 1 and current_col = n_cols - 1 and write_pixel = 1 then
                            -- Done generating pattern
                            current_row := 0;
                            current_col := 0;
                            write_pixel := 0;
 
                        elsif current_col = n_cols - 1  and write_pixel = 1 then
                            -- Go to next row
                            current_row := current_row + 1;
                            current_col := 0;
                            write_pixel := 0;

                        elsif write_pixel = 1 then
                            mem_write_data <= (others => '1'); -- Remember to make generic for any size of mem_write_data
                            -- Go to next col
                            ...
                            mem_write_data <= (others => '0');
                            -- Switch between erasing and writing
                            write_pixel := 1;
                        end if;
