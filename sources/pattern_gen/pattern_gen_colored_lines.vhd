...
architecture cool of pattern_gen is
    ...
    signal write_col : integer range 0 to n_cols - 1 := 0;
                    ...
                    generating_pattern <= '1';
                        ...
                        case state is
                            ...
                            when write_pixels =>
                                ...
                                if current_col = write_col then
                                    -- mem_write_data <= std_logic_vector(to_unsigned(current_row, 8)) & x"0000";
                                    if current_row > 32 then
                                        mem_write_data <= x"00" & std_logic_vector(to_unsigned(current_row, 8)) & std_logic_vector(to_unsigned(current_col, 8));
                                    else
                                        mem_write_data <= std_logic_vector(to_unsigned(current_row, 8)) & std_logic_vector(to_unsigned(current_col, 8)) & x"00";
                                    end if;
                                    -- mem_write_data <= x"FFFFFF";
                                else
