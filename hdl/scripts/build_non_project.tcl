...
if {$status} {
    puts "INFO: Tcl file build_no_project.tcl is valid. All files required for project creation is accesable. "
} else {
    puts "ERROR: Tcl file build_no_project.tcl is not valid. Not all files required for project creation is accesable. "
    return
}
...
set_property -name "board_part" -value "krtkl.com:snickerdoodle_black:part0:1.0" [current_project]
set_property -name "target_language" -value "Verilog" [current_project]
...
read_xdc $constraints_pin_path
read_xdc $constraints_debug_path
...
read_verilog -library work [make_wrapper -fileset sources_1 -import -top -files [get_files -norecurse design_1.bd]]
...
write_hw_platform -force $path_out/hw-platform.xsa -include_bit
